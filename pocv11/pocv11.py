...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v18 (Silicon Scaffold)")
        ...
        # SCHEMATIC SETTINGS
        self.schem_engine = "Auto" # Auto, Graphviz, NetlistSVG
        self.schem_theme = "Standard" # Standard, Dark, Blueprint
        ...
        self.root.bind("<grave>", self.activate_sk_mode); self.editor.text.bind("<grave>", self.activate_sk_mode); self.term_input.bind("<grave>", self.activate_sk_mode)
        self.editor.text.bind("<Key>", self.handle_widget_key_intercept); self.term_input.bind("<Key>", self.handle_widget_key_intercept); self.root.bind("<Key>", self.handle_widget_key_intercept)

        self.root.bind("<F1>", lambda e: self.run_simulation()); self.root.bind("<F2>", lambda e: self.open_waves()); self.root.bind("<F3>", lambda e: self.generate_schematic())

        self.refresh_file_tree(); self.update_ui_labels()
        self.log_system(f"Silis v18 Ready. CWD: {self.cwd}", "SYS")
        self.check_dependencies()
        self.root.after(100, self.process_queue)

    def check_dependencies(self):
        """ Checks for NetlistSVG capability """
        has_node = shutil.which("netlistsvg") is not None
        has_rsvg = shutil.which("rsvg-convert") is not None
        if has_node and has_rsvg:
            self.log_system("NetlistSVG Engine: Ready (Pro Mode Available)", "SUCCESS")
        else:
            self.log_system("NetlistSVG Engine: Not Found (Falling back to Graphviz)", "WARN")
    ...
    def open_settings(self):
        ...
        def save():
            ...
            # If schematic exists, regenerate with new theme immediately
    ...
    def activate_sk_mode(self, event):
        ...
        self.root.after(self.sk_timeout, self.reset_sk); return "break"
    def reset_sk(self): self.sk_active = False; self.root.config(cursor="arrow")
    ...
    def create_toolbar(self):
        toolbar = tk.Frame(self.root, bd=1, relief=tk.RAISED, bg="#e1e1e1"); toolbar.pack(side=tk.TOP, fill=tk.X)
    ...
    def on_tree_double_click(self, event):
        ...
        else: parent_dir = os.path.dirname(path); self.change_directory_silent(parent_dir); self.open_file_in_editor(path)
    ...
    def generate_schematic(self):
        ...
        # DETERMINE ENGINE
        use_netlist = False
        if self.schem_engine == "NetlistSVG": use_netlist = True
        elif self.schem_engine == "Auto":
            if shutil.which("netlistsvg") and shutil.which("rsvg-convert"): use_netlist = True

        if use_netlist:
            self.generate_netlistsvg(synth_files)
        else:
            self.generate_graphviz(synth_files)

    def generate_graphviz(self, synth_files):
        self.log_system("Generating Schematic (Graphviz)...", "SYS")
        ...
        dot_path = os.path.join(self.cwd, "schematic.dot")
        if os.path.exists(dot_path):
            # APPLY THEME & SHAPES PATCH
            self.patch_graphviz_style(dot_path)
            # RENDER
    ...
    def patch_graphviz_style(self, dot_path):
        """ Injects Colors and Approximated Shapes into .dot """
        with open(dot_path, 'r') as f: content = f.read()
        
        # THEMES
        if self.schem_theme == "Dark":
            content = content.replace('graph [', 'graph [bgcolor="#1e1e1e", fontcolor="white", ')
            content = content.replace('node [', 'node [color="#00FFFF", fontcolor="white", ')
            content = content.replace('edge [', 'edge [color="white", ')
        elif self.schem_theme == "Blueprint":
            content = content.replace('graph [', 'graph [bgcolor="#003366", fontcolor="white", ')
            content = content.replace('node [', 'node [color="white", fontcolor="white", ')
            content = content.replace('edge [', 'edge [color="white", ')
        
        # IEEE SHAPE APPROXIMATION (Regex magic)
        # Remap $_AND_ to box with blue fill, NOT to triangle, etc.
        # This is a basic patcher.
        if "Standard" not in self.schem_theme: # Apply fills if themed
             ...
             content = re.sub(r'label="\$_(DFF)_', r'shape=square, style=filled, fillcolor="#B16286", label="$_\1_', content)

        with open(dot_path, 'w') as f: f.write(content)

    def generate_netlistsvg(self, synth_files):
        self.log_system("Generating Schematic (NetlistSVG)...", "SYS")
        # 1. Yosys -> JSON
        cmd_json = f"yosys -p 'read_verilog {' '.join(synth_files)}; hierarchy -auto-top; proc; opt; write_json schematic.json' -q"
        subprocess.run(cmd_json, shell=True, cwd=self.cwd)
        
        if os.path.exists(os.path.join(self.cwd, "schematic.json")):
            # 2. JSON -> SVG (NetlistSVG)
            skin = ""
            if self.schem_theme == "Dark": skin = "--skin default" # NetlistSVG default is actually dark-ish/clean
            # In real usage, you'd pass a custom skin file.
            
            subprocess.run(f"netlistsvg schematic.json -o schematic.svg {skin}", shell=True, cwd=self.cwd)
            
            # 3. SVG -> PNG (rsvg-convert)
            if os.path.exists(os.path.join(self.cwd, "schematic.svg")):
                # Theme Injection for SVG (Simple Color Swap)
                if self.schem_theme == "Blueprint":
                    with open(os.path.join(self.cwd, "schematic.svg"), 'r') as f: svg = f.read()
                    svg = svg.replace('<svg ', '<svg style="background-color: #003366;" ')
                    svg = svg.replace('stroke="black"', 'stroke="white"')
                    with open(os.path.join(self.cwd, "schematic.svg"), 'w') as f: f.write(svg)
                
                subprocess.run("rsvg-convert schematic.svg -o schematic.png", shell=True, cwd=self.cwd)
                self.schematic_viewer.load_image(os.path.join(self.cwd, "schematic.png"))
            else: self.log_system("Error: NetlistSVG failed.", "ERROR")
        else: self.log_system("Error: Yosys JSON gen failed.", "ERROR")
