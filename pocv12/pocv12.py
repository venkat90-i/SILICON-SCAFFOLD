...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v22 (Cursor Fixed)")
        self.root.geometry("1400x900")
        self.root.config(cursor="arrow") # Force Arrow on Init
        ...
        self.root.bind("<Control-n>", lambda e: self.new_file()); self.root.bind("<Control-s>", lambda e: self.save_file())

        # SUPERKEY ACTIVATORS
        for w in [self.root, self.editor.text, self.term_input, self.tree]:
            w.bind("<grave>", self.activate_sk_mode)
        
        # SUPERKEY INTERCEPTORS
        for w in [self.root, self.editor.text, self.term_input, self.tree, self.schematic_viewer.canvas]:
            w.bind("<Key>", self.handle_widget_key_intercept)
    ...
    def activate_sk_mode(self, event):
        self.sk_active = True
        # VISUAL INDICATOR instead of Cursor change
        self.sk_status_label.config(text=" SK ACTIVE ", bg="#50FA7B", fg="black") 
        self.root.after(self.sk_timeout, self.reset_sk)
        return "break"

    def reset_sk(self):
        self.sk_active = False
        self.sk_status_label.config(text=" SK ", bg="#333", fg="gray")
    ...
    def create_toolbar(self):
        ...
        # SK INDICATOR
        self.sk_status_label = tk.Label(toolbar, text=" SK ", bg="#333", fg="gray", font=("Arial", 8, "bold"))
        self.sk_status_label.pack(side=tk.LEFT, padx=10)
    ...
    def generate_schematic(self):
        ...
        if use_netlist: self.generate_netlistsvg(synth_files)
        else: self.generate_graphviz(synth_files)

    def generate_graphviz(self, synth_files):
        self.log_system("Generating Schematic (Graphviz)...", "SYS")
        cmd = f"yosys -p 'read_verilog {' '.join(synth_files)}; hierarchy -auto-top; proc; opt; autoname; show -format dot -prefix schematic' -q"
        ...
        if os.path.exists(dot_path):
            ...
            subprocess.run("dot -Tpng schematic.dot -o schematic.png", shell=True, cwd=self.cwd, capture_output=True)
    ...
    def generate_netlistsvg(self, synth_files):
        ...
        cmd_json = f"yosys -p 'read_verilog {' '.join(synth_files)}; hierarchy -auto-top; proc; opt; autoname; write_json schematic.json' -q"
        subprocess.run(cmd_json, shell=True, cwd=self.cwd, capture_output=True)
        if os.path.exists(os.path.join(self.cwd, "schematic.json")):
            subprocess.run(f"netlistsvg schematic.json -o schematic.svg", shell=True, cwd=self.cwd, capture_output=True)
            svg_path = os.path.join(self.cwd, "schematic.svg")
            if os.path.exists(svg_path):
                self.inject_svg_css(svg_path)
                subprocess.run("rsvg-convert schematic.svg -o schematic.png", shell=True, cwd=self.cwd, capture_output=True)
                self.schematic_viewer.load_image(os.path.join(self.cwd, "schematic.png"))
