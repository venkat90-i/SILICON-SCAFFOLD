...
from PyQt6.QtGui import (QAction, QFont, QColor, QSyntaxHighlighter, 
                         ...
                         QTransform, QPolygonF, QIcon, QPainterPath)
...
class DEFParser:
    def __init__(self, def_path):
        ...
        self.nets = []       
        self.power_rails = [] 
        # Changed: power_routes is now a list of lists: [ (width, [p1, p2]), (width, [p3, p4]) ]
        self.power_routes = [] 
        self.dbu = 1000.0    
    ...
    def parse(self):
        ...
        for line in lines:
            try:
                ...
                # --- SPECIALNETS (Power) ---
                    ...
                    # ROUTED Handling with NEW support
                    if "ROUTED" in line:
                        parsing_route = True
                        # If a previous route segment exists, save it
                        if len(current_route_points) >= 2:
                            self.power_routes.append((current_route_width, current_route_points))
                        current_route_points = []
                    ...
                    if parsing_route:
                        # Check for NEW keyword (Split Segment)
                        if "NEW" in line:
                            if len(current_route_points) >= 2:
                                self.power_routes.append((current_route_width, current_route_points))
                            current_route_points = []

                        # Check for End of Net
        ...
        print(f"DEBUG: Parsed {self.component_count} comps, {len(self.pins)} pins, {len(self.power_rails)} rects, {len(self.power_routes)} route-segments.")
        ...
        try:
            ...
            if self.show_heatmap:
                ...
                if self.show_power:
                    ...
                    # Draw ROUTED (Rails) - SCALED DOWN
                    # Calculate a "nice" thin width relative to die size
                    ...
                    for width, points in self.def_data.power_routes:
                        ...
                        # Use thin width instead of huge DEF width for visibility
