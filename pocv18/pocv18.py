# !!!!CRASHES WITH SDC FILE ISSUES, LATEST HALF_STABLE!!!!
...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v39 (Report Fix)")
        ...
        self.log_system(f"Silis v39 Ready. CWD: {self.cwd}", "SYS")
    ...
    def run_synthesis_flow(self):
        ...
        if not os.path.exists(sdc_path):
             ...
             if not sdc_data: 
                 self.log_system("Synthesis Cancelled (No SDC).", "WARN"); return
        ...
        try:
            ...
            if res.returncode != 0:
                self.log_system("Synthesis Failed.", "ERROR"); return
            ...
            if area_match:
                ...
                self.log_system(f"Area: {area} sq units", "SUCCESS")
        except Exception as e: self.log_system(f"Synth Exception: {e}", "ERROR"); return

        # 3. OpenSTA
        self.log_system("OpenSTA Analysis...", "SYS")
        tcl_script = f"""
        ...
        try:
            ...
            self.show_implementation_summary(log_path)
        except Exception as e:
            self.log_system(f"STA Failed: {e}", "ERROR")

    def show_implementation_summary(self, log_path):
        wns = "N/A"
        pwr = "N/A"
        ...
        if os.path.exists(log_path):
            ...
            # --- FIX: DETECT NO PATHS ---
            if "No paths found" in content:
                wns = "No Paths (Comb.)"
            else:
                match = re.search(r"slack\s+\((.*?)\)\s+([-\d\.]+)", content)
                if match:
                    status, val = match.groups()
                    wns = f"{val} ({status})"
            
            # --- FIX: IMPROVED POWER REGEX FOR OPENSTA TABLE ---
            # Matches: Total   <Internal> <Switching> <Leakage> <TOTAL_POWER>
