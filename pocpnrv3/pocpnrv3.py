...
class DEFParser:
    ...
    def parse(self):
        ...
        for line in lines:
            try:
                ...
                # --- PARSING ---
                    ...
                    # Sticky Logic: Look for placement on this line OR following lines
                    if current_pin_name:
                        if "PLACED" in line or "FIXED" in line:
                            ...
                            if coord_match:
                                x = int(coord_match.group(1))
                                y = int(coord_match.group(2))
                                pin_sz = 1 * self.dbu # Make pins visible
                                self.pins.append((QRectF(x, y, pin_sz, pin_sz), current_pin_name))
                                current_pin_name = None # Done with this pin

                elif current_section == "SPECIALNETS":
                    # Power rails are usually RECTs in SPECIALNETS
                    if "RECT" in line:
                        nums = re.findall(r'(-?\d+)', line)
                        if len(nums) >= 4:
        ...
        print(f"DEBUG: Parsed {self.component_count} comps, {len(self.pins)} pins, {len(self.power_rails)} power rails.")

# ================= 2. SILICON PEEKER (Your Colors + Fixes) =================
...
class SiliconPeeker(QGraphicsView):
    ...
    def redraw(self):
        try:
            ...
            # HEATMAP MODE (Your Custom Settings)
            if self.show_heatmap:
                ...
                if self.show_insts:
                    ...
                    for name, rect in self.def_data.comps_map.items():
                        ...
                        if ctype == "TAP":
                            item.setPen(QPen(Qt.PenStyle.NoPen)) 
                            item.setBrush(QBrush(QColor("#ddddhh"))) # Your Lavender
                        else:
            ...
            if self.show_pins:
                for rect, name in self.def_data.pins:
                    ...
                    # Make triangle size relative to die so they don't vanish
                    sz = max(5 * self.def_data.dbu, d.width() / 100) 
    ...
    def draw_organic_heatmap(self, die_rect):
        # Your Custom Heatmap Logic
        expansion = 4 * self.def_data.dbu 
