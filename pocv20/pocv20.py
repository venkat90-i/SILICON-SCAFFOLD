...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v50 (Complete Fix)")
        ...
        self.term_frame = tk.LabelFrame(self.main_split, text="Dashboard (F4=System Term | F1=App Term)"); self.main_split.add(self.term_frame, weight=1)
        ...
        self.log_system(f"Silis v50 Ready. CWD: {self.cwd}", "SYS")
        self.check_dependencies()
        self.root.after(100, self.update_log_view)
    ...
    def stop_process(self):
        if self.process:
            ...
            if hasattr(self.process, 'kill'): self.process.kill()
            self.process = None
    ...
    # =========================================================================
    #                    SYNTHESIS ENGINE (F4) + OPENSTA (PASSTHROUGH)
    # =========================================================================
    ...
    def run_synthesis_flow(self):
        ...
        self.log_system("NOTE: Check your main terminal for live tool output.", "WARN")
        ...
        # 2. Yosys Synthesis (With TEE to reports/area.rpt)
        ...
        # 3. OpenSTA (Tee Strategy)
        # Using standard Tcl output (no redirects inside Tcl), handled by shell pipe
        tcl_script = f"""
        ...
        # 4. THREADED EXECUTION (Shell Pipe to File + Console)
        def synth_thread():
            try:
                # RUN YOSYS (Passthrough + File)
                subprocess.run(["yosys", "-p", cmd_synth], cwd=work_dir)
                
                # RUN STA (Passthrough + Pipe to File)
                # cmd: sta reports/sta.tcl | tee reports/sta.log
                # We use shell=True for the pipe to work
                cmd_sta = f"sta reports/sta.tcl | tee reports/sta.log"
                subprocess.run(cmd_sta, shell=True, cwd=work_dir)
                
                # UPDATE GUI SAFELY
                self.root.after(0, lambda: self.finish_synthesis(work_dir, True))
                
            except Exception as e:
                print(f"Exception in thread: {e}")
                self.root.after(0, lambda: self.finish_synthesis(work_dir, False))

        threading.Thread(target=synth_thread, daemon=True).start()

    def finish_synthesis(self, work_dir, success):
        self.set_running_state(False)
        if success:
            self.log_system("Implementation Complete.", "SUCCESS")
            self.parse_and_show_summary(work_dir)
            # AUTO-OPEN REPORTS
            log_path = os.path.join(work_dir, "reports", "sta.log")
            if os.path.exists(log_path): self.open_file_in_editor(log_path)
        else:
            self.log_system("Implementation Failed (See terminal)", "ERROR")

    def parse_and_show_summary(self, work_dir):
        wns = "N/A"
        pwr = "N/A"
        area = "N/A"
        
        # Parse Area (JSON format from tee -o)
        area_path = os.path.join(work_dir, "reports", "area.rpt")
        if os.path.exists(area_path):
            with open(area_path, 'r') as f:
                content = f.read()
                # Parse JSON area from Yosys report
                try:
                    ...
                    if "design" in data and "area" in data["design"]:
                        ...
                        area = data["modules"][key]["area"]
                except:
                    match = re.search(r"\"area\":\s+([\d\.]+)", content)
                    if match: area = match.group(1)

        # Parse STA Log (Combined Timing/Power)
        log_path = os.path.join(work_dir, "reports", "sta.log")
        ...
        summary = f"\n[IMPL SUMMARY]\nArea:           {area} sq units\nTiming (Slack): {wns}\nTotal Power:    {pwr}\n\n(Full logs are in your terminal window)\n"
        self.term_log.insert(tk.END, summary, "SYS"); self.term_log.see(tk.END)

    def open_constraint_wizard(self, module_name):
        wiz = tk.Toplevel(self.root); wiz.title("Constraint Wizard"); wiz.geometry("400x550")
        ...
        # NEW FIELD: DRIVING CELL
        edrive = add_field("Driving Cell", "Lib cell driving inputs (e.g. sky130_fd_sc_hd__inv_1)")
        edrive.insert(0, "sky130_fd_sc_hd__inv_1") # SKY130 DEFAULT
        ...
        def generate():
            ...
            # DRIVING CELL GENERATION (FIXED: NO -library, safe generic logic)
            drive_cell = fields["Driving Cell"].get()
            if drive_cell:
                # We use [all_inputs] because it's safe. OpenSTA will just warn about the clock port.
                sdc += f"set_driving_cell -lib_cell {drive_cell} [all_inputs]\n"
    ...
    def run_simulation(self):
        # HYBRID MODE: F1 COMPILE -> CAPTURE OUTPUT TO APP TERMINAL
        if self.is_running_job: return
        ...
        # Check if current file is the TB
        ...
        try:
            ...
            if not os.path.exists(read_path): 
                 self.log_system(f"Error: TB {tb_file} not found in source/", "ERROR"); self.set_running_state(False); return
            ...
            if not compile_list:
                self.log_system("Error: No sources found for compile.", "ERROR"); self.set_running_state(False); return

            outfile = f"{target_base}.out"
            cmd = ["iverilog", "-o", outfile] + compile_list
            self.log_system(f"Compiling...", "SYS")
            
            # --- CAPTURE OUTPUT FOR F1 ---
            def compile_thread():
                try:
                    res = subprocess.run(cmd, cwd=work_dir, capture_output=True, text=True)
                    if res.stdout: self.queue.put(res.stdout)
                    if res.stderr: self.queue.put(res.stderr)
                    
                    if os.path.exists(os.path.join(work_dir, temp_tb)): os.remove(os.path.join(work_dir, temp_tb))
                    
                    if res.returncode != 0: 
                        self.queue.put("[ERROR] Compilation Failed.\n")
                    else:
                        self.queue.put("[SUCCESS] Running Simulation...\n")
                        # RUN VVP (PASSTHROUGH TO SYSTEM TERM FOR SIM INTERACTION)
                        subprocess.run(["vvp", outfile], cwd=work_dir)
                except Exception as e: self.queue.put(f"[ERROR] {e}\n")
                finally: self.queue.put("STOP_SIGNAL")

            self.process = threading.Thread(target=compile_thread, daemon=True)
            self.process.start()
            self.root.after(50, self.update_log_view) # Enable the firehose for F1
    ...
    def run_simple_compile(self, work_dir, target_base):
        ...
        outfile = f"{target_base}.out"
        # CAPTURE OUTPUT
        res = subprocess.run(["iverilog", "-o", outfile] + all_v, cwd=work_dir, capture_output=True, text=True)
        if res.stdout: self.term_log.insert(tk.END, res.stdout)
        if res.stderr: self.term_log.insert(tk.END, res.stderr, "ERROR")
        if res.returncode != 0: self.log_system("SYNTAX ERROR", "ERROR")
    ...
    def generate_schematic(self):
            ...
            try:
                if use_netlist:
                    ...
                    if os.path.exists(os.path.join(work_dir, f"{prefix}.json")):
                        subprocess.run(f"netlistsvg {prefix}.json -o {prefix}.svg", shell=True, cwd=work_dir)
                        if os.path.exists(os.path.join(work_dir, f"{prefix}.svg")):
                            self.inject_svg_css(os.path.join(work_dir, f"{prefix}.svg"))
                            subprocess.run(f"rsvg-convert {prefix}.svg -o {prefix}.png", shell=True, cwd=work_dir)
                    ...
                    if os.path.exists(os.path.join(work_dir, f"{prefix}.dot")):
                        ...
                        subprocess.run(f"dot -Tpng {prefix}.dot -o {prefix}.png", shell=True, cwd=work_dir)
                        self.queue.put("LOAD_IMAGE")
        ...
        self.process = threading.Thread(target=schem_thread, daemon=True)
        self.process.start()
        self.root.after(50, self.update_log_view)

    def finish_schematic(self, work_dir, prefix):
        path = os.path.join(work_dir, f"{prefix}.png")
        if os.path.exists(path):
            self.schematic_viewer.load_image(path)
            self.log_system("Schematic Loaded.", "SUCCESS")
        else:
            self.log_system("Schematic Generation Failed.", "ERROR")
        self.set_running_state(False)
    ...
    def run_process_threaded(self, command, cwd):
        def target():
            try:
                # PASSTHROUGH TO TERMINAL (No freeze)
                self.process = subprocess.Popen(command, stdin=subprocess.PIPE, cwd=cwd)
                self.process.wait()
            ...
            while not self.queue.empty():
                ...
                if msg == "STOP_SIGNAL":
                    ...
                    if target_base:
                        ...
                        if os.path.exists(path): self.schematic_viewer.load_image(path)
                elif msg == "LOAD_IMAGE":
                    pass # Handled in STOP_SIGNAL check to ensure file is ready
                else:
                    lines.append(msg)
