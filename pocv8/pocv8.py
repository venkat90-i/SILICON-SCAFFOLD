...
def bridge_numpad(widget):
    """ Forces Numpad keys to send standard Arrow events """
    key_map = {
...
class VerilogIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("WSL Verilog Workbench v13 (Leak Fix)")
        ...
        # --- FIX: LEAK PREVENTION ---
        # We bind KEY to the widgets directly to intercept events if SK is active
        ...
        # Intercept ALL keys on editor and terminal to check for SK shortcuts
        self.editor.text.bind("<Key>", self.handle_widget_key_intercept)
        self.term_input.bind("<Key>", self.handle_widget_key_intercept)
        
        # Fallback for Root (in case focus is elsewhere)
        self.root.bind("<Key>", self.handle_widget_key_intercept)

        self.refresh_file_tree()
        self.log_system(f"Workbench v13 Ready. CWD: {self.cwd}")
        self.root.after(100, self.process_queue)

    # ================= SETTINGS =================
    def open_settings(self):
        win = tk.Toplevel(self.root); win.title("Settings"); win.geometry("300x350")
        tk.Label(win, text="Superkey (` + ...)", font=("Arial", 10, "bold")).pack(pady=10)
        ...
        frame_t = tk.Frame(win); frame_t.pack(fill=tk.X, padx=10, pady=15)
        ...
        def save():
            ...
            self.log_system(f"Settings Saved. Timeout: {self.sk_timeout}ms")
            win.destroy()
    ...
    def handle_widget_key_intercept(self, event):
        """ The Master Key Handler. If SK Active, swallow key. If not, let it pass. """
        if not self.sk_active: return None # Let normal typing happen
        ...
        elif key == self.key_map["term_toggle"]:
            self.toggle_term_mode()
            self.log_system("Mode Toggled")
            
        self.reset_sk()
        return "break" # CRITICAL: Stops event from reaching the widget!

    # ================= AUTOCOMPLETE =================
    def handle_arrow_autocomplete(self, event):
        if self.term_mode != "SHELL": return
        full_text = self.term_input.get(); cursor_pos = self.term_input.index(tk.INSERT)
        ...
        if len(matches) == 1:
            ...
            self.term_input.delete(0, tk.END); self.term_input.insert(0, base + completion); return "break"
        return
    ...
    def on_tree_enter_key(self, event):
        """ FIX: Enter now Forces CD and Toggles """
        item_id = self.tree.focus()
        if not item_id: return
        path, ftype = self.tree.item(item_id, "values")
    ...
    def on_tree_up_dir(self, event):
        try: os.chdir(".."); self.cwd = os.getcwd(); self.refresh_file_tree(); self.log_system(f"CD .. -> {self.cwd}")
        except Exception as e: self.log_system(f"Error: {e}")
