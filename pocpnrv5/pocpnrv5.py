...
class DEFParser:
    ...
    def parse(self):
        ...
        current_route_points = [] # List of QPointF
        parsing_route = False
        
        # Routing Context for Wildcards
        last_x = 0
        last_y = 0
        ...
        for line in lines:
            try:
                ...
                # --- NETS & SPECIALNETS (UNIFIED ROUTING PARSER) ---
                elif current_section in ["NETS", "SPECIALNETS"]:
                    if "ROUTED" in line:
                        parsing_route = True
                        # Flush previous wire if exists
                        if len(current_route_points) >= 2:
                    ...
                    if parsing_route:
                        ...
                        # Stop if end of statement definition
                        ...
                        # --- THE WILDCARD PARSER ---
                        # Extract everything inside parens: ( 1000 1000 ) ( * 2000 )
                        if "(" in line:
                            tokens = line.replace("(", " ").replace(")", " ").split()
                            # Iterate tokens to find coordinates
                            i = 0
                            while i < len(tokens):
                                t = tokens[i]
                                # Check if it might be a coordinate start (number or *)
                                if t == "*" or t.lstrip('-').isdigit():
                                    # We need a pair (x, y). Check if next token exists.
                                    if i + 1 < len(tokens):
                                        val_x_str = t
                                        val_y_str = tokens[i+1]
                                        
                                        # Parse X
                                        if val_x_str == "*": 
                                            x = last_x
                                        elif val_x_str.lstrip('-').isdigit(): 
                                            x = int(val_x_str)
                                        else: 
                                            i += 1; continue # Not a coord

                                        # Parse Y
                                        if val_y_str == "*": 
                                            y = last_y
                                        elif val_y_str.lstrip('-').isdigit(): 
                                            y = int(val_y_str)
                                        else: 
                                            i += 1; continue # Not a coord
...
class SchematicWorker(QThread):
    ...
    def run(self):
        ...
        # UPDATED COMMAND: "proc; opt" only. NO "techmap", NO "abc".
        # This keeps the schematic at the Register/Mux/Adder level (RTL view)
        # instead of exploding into 10k NAND gates.
        cmd_dot = f"yosys -p '{read_cmd} hierarchy -check -top {self.base}; proc; opt; show -width -stretch -format dot -prefix {self.base}'"
        ...
        if os.path.exists(dot_path):
            try:
                # Add a timeout to dot to prevent freezing on massive designs
                subprocess.run(f"dot -Tsvg {dot_path} -o {self.base}.svg", shell=True, cwd=self.root, timeout=10)
                self.finished.emit(os.path.join(self.root, f"{self.base}.svg"))
            except subprocess.TimeoutExpired:
                 self.log.emit("Schematic too complex to render (Timeout)", "WARN")
            except Exception as e:
                 self.log.emit(f"Graphviz Failed: {e}", "ERROR")
