# WORKS. Havent test with false sdc yet.
...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v52 (Path Fix)")
        ...
        self.log_system(f"Silis v52 Ready. CWD: {self.cwd}", "SYS")
    ...
    def get_project_root(self, target_base):
        ...
        # Case 1: CWD is the project root
        if os.path.basename(cwd_abs) == project_dir_name: 
            return cwd_abs
        # Case 2: CWD is a subdir of project root (source, netlist, etc)
        ...
        # Case 3: Project needs to be created in CWD
        else: 
            return os.path.join(cwd_abs, project_dir_name)
    ...
    def ensure_project_workspace(self, target_base):
        ...
        # Search relative to initial CWD if not already in project
    ...
    def create_safe_sdc_if_missing(self, work_dir, target_base):
        sdc_path = os.path.join(work_dir, "netlist", f"{target_base}.sdc")
        if not os.path.exists(sdc_path):
            self.log_system(f"SDC missing. Creating safe default for {target_base}...", "WARN")
            default_sdc = f"""# SDC Constraints for {target_base}
# Auto-generated by Silis v51 (Safe Mode)

# 1. Units (Sky130 default)
set_units -time ns -resistance kOhm -capacitance pF -voltage V -current mA

# 2. Clock (Edit period as needed)
...
# 4. Driving Cell (Safe Generic)
# Using generic buffer/inverter logic to avoid library mismatch
# set_driving_cell -lib_cell sky130_fd_sc_hd__inv_1 [all_inputs]

# 5. Load (0.05pF = 50fF, realistic)
set_load 0.05 [all_outputs]
"""
            with open(sdc_path, 'w') as f: f.write(default_sdc)
            self.open_file_in_editor(sdc_path)
            self.log_system("Please verify SDC and run F4 again.", "SYS")
            return False # Stop flow to let user verify
        return True
    ...
    def run_synthesis_flow(self):
        ...
        # 1. Check/Create SDC
        if not self.create_safe_sdc_if_missing(work_dir, target_base): return

        self.set_running_state(True)
        self.btn_errors.config(state="disabled") # Reset error button
        self.violation_log.delete(1.0, tk.END) # Clear violations
...
"""
        ...
        def flow_thread():
            try:
                ...
                cmd_sta = f"sta sta.tcl | tee reports/sta.log"
                ...
                self.queue.put(("HARVEST_ERRORS", work_dir)) # Still harvest warnings
                self.queue.put("PARSE_SUMMARY") # Show dashboard metrics
    ...
    def harvest_logs(self, work_dir=None):
        ...
        # If work_dir is passed via queue, use it. Else calculate it.
        if not work_dir:
            _, target_base = self.get_context_target()
        ...
        if not os.path.exists(report_dir):
            ...
            except: pass # Can't write log if we can't make dir
                ...
                for log in logs:
                    ...
                    if os.path.exists(path):
                        ...
                        with open(path, 'r') as infile:
                            for line in infile:
                                # Harvester Logic: Filter for Error or Warning
        ...
        if found_issues:
            self.btn_errors.config(state="normal") # Enable panic button
    ...
    def load_violation_log(self):
        """Manual trigger for panic button"""
        self.harvest_logs()
