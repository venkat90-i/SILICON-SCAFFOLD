...
class VerilogIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("WSL Verilog Workbench v16 (Smart Autocomplete)")
        ...
        self.term_log = scrolledtext.ScrolledText(self.term_frame, bg="#1e1e1e", fg="#00ff00", font=("Consolas", 10), height=8)
        ...
        self.log_system(f"Workbench v16 Ready. CWD: {self.cwd}")
    ...
    def open_settings(self):
        ...
        def save():
            ...
            self.update_ui_labels()
            self.log_system(f"Settings Saved & UI Updated.")
            win.destroy()
    ...
    def handle_widget_key_intercept(self, event):
        ...
        elif key == self.key_map["term_toggle"]: self.toggle_term_mode(); self.log_system("Mode Toggled")
        self.reset_sk(); return "break"

    # ================= TAB AUTOCOMPLETE (NESTED SUPPORT) =================
    def handle_tab_autocomplete(self, event):
        if self.term_mode != "SHELL": return "break"
        
        full_text = self.term_input.get()
        # 1. Extract Last Word (Prefix)
        if " " in full_text: base, prefix = full_text.rsplit(" ", 1); base += " "
        else: base, prefix = "", full_text
        
        # 2. Split into Dir and Partial File
        # e.g. "src/mo" -> head="src", tail="mo"
        # e.g. "mo" -> head="", tail="mo"
        head, tail = os.path.split(prefix)
        
        # 3. Determine Search Directory
        # If head is empty, look in CWD. If head exists, look inside it.
        ...
        try:
            ...
            # 4. Filter Matches
            candidates = [f for f in os.listdir(search_dir) if f.startswith(tail)]
            
            # 5. Handle Results
            if len(candidates) == 1:
                # Unique Match -> Append to Base + Head
                match = candidates[0]
                ...
                self.log_system(f"Options: {', '.join(candidates)}")
                
        except Exception as e:
            self.log_system(f"Autocomplete Error: {e}")
