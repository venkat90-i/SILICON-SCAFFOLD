...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v23 (Auto-Organizer)")
        ...
        self.log_system(f"Silis v23 Initialized. CWD: {self.cwd}", "SYS")
    ...
    # =========================================================================
    #                    PROJECT ORGANIZER LOGIC (THE BRAIN)
    # =========================================================================
    
    def get_context_target(self):
        """ Returns (module_name, target_base_name) """
    ...
    def ensure_project_folder(self, target_base):
        """ 
        Moves files to a project subfolder if they are in the root. 
        Returns the path to the project folder.
        """
        # If we are already deep in a folder structure, assume we are fine
        # We only auto-organize if we are in the root or close to it
        ...
        if self.current_file and os.path.dirname(self.current_file) == self.cwd:
            # File is in root -> Move it
            file_name = os.path.basename(self.current_file)
            new_path = os.path.join(project_path, file_name)
            try:
                # Save first to ensure disk is current
        ...
        # 3. Check associated files (TB or Design)
    ...
    def run_simulation(self):
        if self.current_file: self.save_file() # Save current state
        ...
        # === TRIGGER ORGANIZER ===
        # This will move files and return the new working directory
        ...
        try:
            ...
            # Compile List: All .v in project folder, exclude orig TB, include temp TB
            ...
            # Run vvp inside the project folder
    ...
    def open_waves(self):
        # Check inside the project folder of current module
        ...
        # Fallback: scan root (unlikely now)
    ...
    def create_toolbar(self):
        ...
        # Clean button removed, moved to Settings
    ...
    def open_settings(self):
        ...
        tk.Label(win, text="Theme", font=("Arial", 10, "bold")).pack(pady=5)
        theme_var = tk.StringVar(value=self.schem_theme)
        ttk.OptionMenu(win, theme_var, self.schem_theme, "Standard", "Dark", "Blueprint").pack()
        ...
        def save():
            ...
            self.schem_engine = eng_var.get(); self.schem_theme = theme_var.get()
